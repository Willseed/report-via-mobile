...
| 版本 / 維護線                    | 是否支援           | 說明 |
| -------------------------------- | ------------------ | ---- |
| `main`                           | :white_check_mark: | 目前唯一受支援的維護線 |
| 歷史快照、功能分支、私人 fork    | :x:                | 不提供安全性修補承諾 |

## 發布與版本策略

本專案會將安全性修補與其他顯著變更記錄在根目錄的 `CHANGELOG.md`，並在建立正式 GitHub
Release 後採用語意化版本（SemVer）管理版本。

目前尚未建立第一個正式 tagged release，因此 `main` 分支是唯一受支援的維護線。未來若新增
正式 release line，會同步更新本文件與 `CHANGELOG.md` 的支援範圍。
