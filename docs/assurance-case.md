...
| SAST flags `localStorage` as information disclosure | Browser UI | Only non-sensitive theme preference (`light` / `dark` / `auto`) is persisted in `localStorage`; report content, phone numbers, tokens, and location are not stored there | XSS could still read that theme string; this is accepted because the value is not confidential. Treat Codacy/OWASP "Information in Browser localStorage" findings on `ThemeService` as false positives |
